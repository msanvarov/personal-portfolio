...
const lineColor = (opacity, props) =>
  props.secondary ? `rgb(var(--rgbTitle) / ${opacity})` : `rgb(var(--rgbPrimary) / ${opacity})`;
...
const Anchor = styled.a.attrs(({ target, rel }) => ({
  ...
  background: linear-gradient(${(props) => lineColor(1, props)}, ${(props) => lineColor(1, props)})
      no-repeat 100% 100% / 0 2px,
    linear-gradient(${(props) => lineColor(0.3, props)}, ${(props) => lineColor(0.3, props)})
