...
// Color utils
const _stringToRGBA = (color: string) => color.split(' ').join('').replace('rgb(', '').replace('rgba(', '').replace(')', '').split(',');

export const tint = (color: string, percent: number) => {
    const colorArray = color.replace('rgba(', '').replace(')', '').split(',');
...
export const rgba = (color: string, alpha: number) => {
    const colorArray = _stringToRGBA(color);
